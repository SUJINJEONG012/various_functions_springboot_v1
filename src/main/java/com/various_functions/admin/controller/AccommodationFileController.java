...
public class AccommodationFileController {
	...
	// private final String uploadPath = "C:\\Users\\NCIN\\single-upload-files"; //
	// 파일이 저장된 경로
	...
	public ResponseEntity<Resource> AdminListFile(@PathVariable final Long accommodationId,
		...
		try {
			...
			return ResponseEntity.ok().contentType(MediaType.IMAGE_JPEG) // 이미지 파일인 경우
	...
	@GetMapping({ "/admin/accommodation/{accommodationId}/files/{afId}/download",
			"/accommodation/{accommodationId}/files/{afId}/download" })
	public ResponseEntity<Resource> downloadFile(@PathVariable final Long accommodationId,
			@PathVariable final Long afId) {
		AccommodationsFileVo file = accommodationFileService.findFileById(afId);
	...
	public ResponseEntity<Resource> UserViewListFile(@PathVariable final Long accommodationId,
			@PathVariable final Long afId) {
